...
class CustomOptimizerConstructor:
    ...
    def add_params(self,
        ...
        # first in, first match
        sorted_keys = custom_keys.keys()
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            for key in sorted_keys:
                if key in f'{prefix}.{name}':
                    ...
                    group_name = f'lr_{lr_mult}_wd_{decay_mult}'
                    if group_name not in custom_params_names:
                        custom_params_names[group_name] = []
                    custom_params_names[group_name].append(f'{prefix}.{name}')
                    break
    ...
    def __call__(self, model: nn.Module):
        ...
        params: List[Dict] = []
        custom_param_names = dict()
        self.add_params(params, custom_param_names, model)
        optimizer_cfg['params'] = params
        print("custom parameter groups: \n%s" % json.dumps(custom_param_names, indent=2))
        return build_from_cfg(optimizer_cfg, OPTIMIZERS)
