...
from mmcv.engine import collect_results_gpu
...
def single_gpu_test(model,
    ...
    """
    # NOTE: It has not been modified for Seq Tracking, should not be called
    model.eval()
...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    for i, data in enumerate(data_loader):
        ...
        for _data in data:
            with torch.no_grad():
                result = model(return_loss=False, rescale=True, **_data)
            for key in result:
                if 'mask' in key:
                    result[key] = encode_mask_results(result[key])
        ...
        if rank == 0 and world_size == 1:
            batch_size = data[0]['img'][0].size(0)
            for _ in range(batch_size * world_size):
                prog_bar.update()

    # collect results from all ranks
    if gpu_collect:
        results = {k: collect_results_gpu(v, len(v)) for k, v in results.items()}
    else:
