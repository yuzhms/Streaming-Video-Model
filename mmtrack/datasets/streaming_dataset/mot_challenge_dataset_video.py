...
from .custom_coco_video import CustomCOCOVideo
...
class MOTChallengeDatasetVideo(CustomCOCOVideo):
    ...
    def _create_data_infos(self):
        ...
        self.full_data_infos = []
        for info in data_infos:
            self.full_data_infos.extend(self.ref_img_sampling(info, **self.ref_img_sampler))
        return data_infos
    ...
    def format_results(self, results, resfile_path=None, metrics=['track']):
        ...
        if 'bbox_bboxes' not in results:
            results['bbox_bboxes'] = results.get('det_bboxes', [])
        ...
        num_frames = [len(self.coco.get_img_ids_from_vid(img_info['video_id'])) for img_info in self.data_infos]
        inds = [0]
        for _n_frame in num_frames:
            inds.append(inds[-1] + _n_frame)
        num_vids = len(inds) - 1
        ...
        for i in range(num_vids):
            for metric in metrics:
                ...
                formatter(results[f'{metric}_bboxes'][inds[i]:inds[i + 1]],
                          self.full_data_infos[inds[i]:inds[i + 1]],
    ...
    def format_track_results(self, results, infos, resfile):
        ...
        if self.interpolate_tracks_cfg is not None and len(results_per_video) > 0:
    ...
    def evaluate(self,
        ...
        if 'bbox' in metrics:
            ...
            #     annotations.extend(self.ref_img_sampling(info))
            # ref_annotations = [self.ref_img_sampling(info) for info in self.data_infos]
            annotations = [self.get_ann_info(info) for info in self.full_data_infos]
            for iou_thr in bbox_iou_thr:
                mean_ap, _ = eval_map(
                    bbox_results,
                    annotations,
                    iou_thr=iou_thr,
