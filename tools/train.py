...
def main():
    ...
    if osp.isdir(cfg.work_dir):
        ...
        checkpoints = [ckpt for ckpt in checkpoints if ckpt.endswith('.pth')
                       and 'latest' not in ckpt and 'best' not in checkpoints]
        print(f"All checkpoints founded in {cfg.work_dir}: {checkpoints}")
        if len(checkpoints) > 0:
            checkpoints.sort(key=lambda x: int(x.split('.')[0].split('_')[-1]))
            latest_checkpoint = checkpoints[-1]
            cfg.resume_from = osp.join(cfg.work_dir, latest_checkpoint)
            for hook in cfg.custom_hooks:
                if hook['type'] == 'ExpMomentumEMAHook' and 'resume_from' in hook:
                    hook['resume_from'] = cfg.resume_from
            print(f"Resume from the latest checkpoint: {cfg.resume_from}")
